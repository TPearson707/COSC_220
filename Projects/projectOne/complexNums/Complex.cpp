...
Complex operator/(double lhs, const Complex& rhs) {
    ...
    Complex result;
    double denominator = (rhs.realNum * rhs.realNum) + fabs((rhs.imagNum * rhs.imagNum));

    result.setReal((left * rhs.real) / denominator);
    result.setImag((left * -rhs.imag) / denominator);
